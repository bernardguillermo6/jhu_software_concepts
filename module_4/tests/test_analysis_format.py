...
def test_buttons_are_present(client):
    """
    GET / should render the 'Pull Data' and 'Update Analysis' buttons
    with stable IDs so they can be targeted in UI tests.
    """
    response = client.get("/")
    ...
    assert fetch_btn is not None, "Expected Pull Data button with id='fetch-btn'"
    assert "Pull Data" in fetch_btn.get_text()

    assert refresh_btn is not None, "Expected Update Analysis button with id='refresh-btn'"
    assert "Update Analysis" in refresh_btn.get_text()
